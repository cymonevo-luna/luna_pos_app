...
const menuPhotoMaxBytes = 5 * 1024 * 1024;

const _allowedExtensions = {'.jpg', '.jpeg', '.png', '.webp'};

/// Validates and uploads a menu photo image.
class MenuPhotoUpload {
  MenuPhotoUpload(this._api);

  final ApiClient _api;

  static const uploadPath = '/api/admin/uploads/menu-photo';
  ...
  static void validate(XFile file, {required int byteLength}) {
    ...
    if (!hasAllowedExtension) {
      throw const MenuPhotoValidationException(
        'Photo must be a JPEG, PNG, or WebP image.',
      );
    }
    if (byteLength > menuPhotoMaxBytes) {
      throw const MenuPhotoValidationException(
...
class MenuPhotoValidationException implements Exception {
  const MenuPhotoValidationException(this.message);
